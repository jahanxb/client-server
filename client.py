...
def client_main(folder_name, concurrency):
    with ThreadPoolExecutor(max_workers=int(concurrency)) as executor:
        ...
        time.sleep(0.2)
        # print("Time taken to upload images using multithreading: {}".format(timeit.Timer(main).timeit(number=1)))
    elapsed = time.time()
        ...
        while True:
            try:
                ...
                for filename in arr:
                    ...
                    Util.checksum(f'{folder_name}/{filename}')

                    image = cv2.imread(f'{folder_name}/{filename}', 0)
